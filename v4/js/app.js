...
(function injectKpmV8Styles(){
  ...
  st.id='kpm-v8-style';
  st.textContent=`
    .item-card.kpm-sortable{position:relative;touch-action:auto}
    .item-drag-handle{width:36px;min-width:36px;height:48px;display:flex;align-items:center;justify-content:center;border-radius:10px;border:1px solid var(--line);background:#fff;color:var(--muted);font-size:22px;font-weight:900;cursor:grab;touch-action:none;user-select:none}
    .item-drag-handle:active{cursor:grabbing;background:#f5ead7}
    .item-card.kpm-dragging{opacity:.68;box-shadow:0 10px 30px rgba(0,0,0,.16);transform:scale(.99)}
...
function initKpmItemSorting(){
  ...
  list.querySelectorAll('.item-drag-handle').forEach(handle=>{
    handle.onpointerdown=(ev)=>{
      if(ev.pointerType==='mouse' && ev.button!==0)return;
      const card=handle.closest('.item-card'); if(!card)return;
      kpmDragCard=card; card.classList.add('kpm-dragging');
      try{handle.setPointerCapture(ev.pointerId);}catch(_){ }
      ev.preventDefault();
    };
    handle.onpointermove=(ev)=>{
      if(!kpmDragCard)return;
      const under=document.elementFromPoint(ev.clientX,ev.clientY)?.closest?.('#itemsList .item-card');
      if(!under||under===kpmDragCard)return;
      const r=under.getBoundingClientRect();
      if(ev.clientY<r.top+r.height/2) list.insertBefore(kpmDragCard,under); else list.insertBefore(kpmDragCard,under.nextSibling);
      ev.preventDefault();
    };
    const finish=async(ev)=>{
      if(!kpmDragCard)return;
      kpmDragCard.classList.remove('kpm-dragging');kpmDragCard=null;
      try{handle.releasePointerCapture(ev.pointerId);}catch(_){ }
      await saveCurrentItemOrder();
    };
