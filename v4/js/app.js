...
async function shareBillPdf(id){
  ...
  try{
    ...
    if(navigator.share && (!navigator.canShare || navigator.canShare({files:[file]}))){
      try{await navigator.share({title:`KPM ${full.rental.bill_no}`,text:full.rental.mobile?`Customer: ${full.rental.customer_name} | Mobile: ${full.rental.mobile}`:`Customer: ${full.rental.customer_name}`,files:[file]});return;}catch(e){if(e?.name==='AbortError')return;console.warn(e);}
    }
    downloadPdfBlob(blob,filename);
    if(full.rental.mobile) await copyCustomerNumber(full.rental.mobile);
    toast('PDF downloaded. Your browser could not open file sharing; customer number has been copied.');
  }catch(e){hideLoading();if(e?.name!=='AbortError')alert(e.message);}
}
...
async function sharePendingPdf(r,pending){
  showLoading('Preparing pending PDF...');
  try{
    const s=await getShopSettings(),blob=await createPendingPdfBlob(r,pending,s),filename=`${r.bill_no}-PENDING.pdf`,file=new File([blob],filename,{type:'application/pdf'});hideLoading();
    if(navigator.share && (!navigator.canShare || navigator.canShare({files:[file]}))){
      try{await navigator.share({title:`KPM Pending Items ${r.bill_no}`,text:r.mobile?`Customer: ${r.customer_name} | Mobile: ${r.mobile}`:`Customer: ${r.customer_name}`,files:[file]});return;}catch(e){if(e?.name==='AbortError')return;console.warn(e);}
    }
    downloadPdfBlob(blob,filename); if(r.mobile) await copyCustomerNumber(r.mobile); toast('Pending PDF downloaded. Customer number copied.');
  }catch(e){hideLoading();if(e?.name!=='AbortError')alert(e.message);}
}
