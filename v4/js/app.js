...
async function createUser(){
  ...
  try{
    ...
    try {
      ...
      throw new Error(raw || `Create user failed (HTTP ${resp.status})`);
    }

    if(!resp.ok || !result.success){
      const rawErr = result?.error ?? result?.message ?? result;
      let msg = 'Could not create user';

      if(typeof rawErr === 'string'){
        msg = rawErr;
      } else if(rawErr && typeof rawErr === 'object'){
        msg = rawErr.message || rawErr.error_description || rawErr.details || rawErr.hint || JSON.stringify(rawErr);
      }
    ...
    else if(e && typeof e === 'object') msg = e.message || e.details || JSON.stringify(e);

    errEl.textContent = msg;
    errEl.style.display='block';
    btn.disabled=false;
    btn.textContent=t('add_user');
  }
}
...
/* =========================================================
   KPM FRONTEND V5 OVERRIDES
========================================================= */

/* ---------- Item photo: camera OR gallery ---------- */
function openItemForm(itemId){
  editingItemId=itemId||null;
  const item=itemId?ITEMS_CACHE.find(i=>i.id===itemId):null;
  pendingItemPhotoFile=null;
  openModal(`<div class="modal-header"><h3>${item?t('edit'):t('add_new_item')}</h3><button class="modal-close" onclick="closeModal()">✕</button></div>
    <div class="photo-upload v5" id="itemPhotoUpload">${item?.photo_url?`<img src="${item.photo_url}">`:`<div class="photo-placeholder"><span style="font-size:32px">🖼️</span><span class="lbl">Item Photo</span><span class="hint">Use Camera or Gallery below</span></div>`}</div>
    <div class="item-photo-actions">
      <button type="button" class="btn btn-outline" onclick="document.getElementById('itemCameraInput').click()">📷 Camera</button>
      <button type="button" class="btn btn-outline" onclick="document.getElementById('itemGalleryInput').click()">🖼️ Gallery / File</button>
    </div>
    <input type="file" accept="image/*" capture="environment" id="itemCameraInput" style="display:none" onchange="handleItemPhotoSelect(event)">
    <input type="file" accept="image/*" id="itemGalleryInput" style="display:none" onchange="handleItemPhotoSelect(event)">
    <div class="field"><label>${t('tamil_name')} *</label><input type="text" id="itemNameInput" value="${item?escapeHtml(item.name):''}" placeholder="தமிழ் பெயர்"></div>
    <div class="field"><label>${t('english_name')}</label><input type="text" id="itemNameEnInput" value="${item?.name_en?escapeHtml(item.name_en):''}" placeholder="English name"></div>
    <div class="row2"><div class="field"><label>${t('total_qty')} *</label><input type="number" id="itemQtyInput" min="0" value="${item?item.total_qty:''}"></div>
    <div class="field"><label>${t('price')} *</label><input type="number" id="itemPriceInput" min="0" step="0.01" value="${item?item.price:''}"></div></div>
...
/* ---------- Simple reports with clear doughnut charts ---------- */
function reportLegendHtml(rows){
  return `<div class="simple-chart-legend">${rows.map(r=>`<div class="simple-legend-row"><div class="left"><span class="dot" style="background:${r.color}"></span><span>${r.label}</span></div><b>${r.value}</b></div>`).join('')}</div>`;
}
function makeSimpleDoughnut(canvasId, labels, values, colors){
  const canvas=document.getElementById(canvasId); if(!canvas||!window.Chart) return null;
  return new Chart(canvas,{type:'doughnut',data:{labels,datasets:[{data:values,backgroundColor:colors,borderWidth:3,borderColor:'#fff',hoverOffset:5}]},options:{responsive:true,maintainAspectRatio:false,cutout:'58%',plugins:{legend:{display:false},tooltip:{callbacks:{label:(ctx)=>`${ctx.label}: ${ctx.raw}`}}},animation:{duration:350}}});
}
async function renderReports(){
  renderReportChips(); showLoading(); const el=document.getElementById('reportsContent');
  try{
    let query=sb.from('rentals').select('*,rental_items(item_name,qty,received_qty)');
    const start=rangeStart(reportRange); if(start) query=query.gte('rental_date',start.toISOString());
    const {data,error}=await query; if(error) throw error;
    const rentals=data||[];
    const totalFinal=rentals.reduce((s,r)=>s+Number(r.final_amount??r.total_amount??0),0);
    const totalReceived=rentals.reduce((s,r)=>s+Number(r.amount_received||0),0);
    const outstanding=Math.max(0,totalFinal-totalReceived);
    const pendingQty=rentals.reduce((s,r)=>s+(r.rental_items||[]).reduce((x,l)=>x+Math.max(0,l.qty-l.received_qty),0),0);
    const active=rentals.filter(r=>r.rental_status==='ACTIVE').length;
    const partial=rentals.filter(r=>r.rental_status==='PARTIALLY_RETURNED').length;
    ...
    const partPaid=rentals.filter(r=>r.payment_status==='PARTIALLY_PAID').length;
    const pendingPay=rentals.filter(r=>r.payment_status==='PENDING').length;
    const itemCounts={}; rentals.forEach(r=>(r.rental_items||[]).forEach(l=>itemCounts[l.item_name]=(itemCounts[l.item_name]||0)+l.qty));
    const top=Object.entries(itemCounts).sort((a,b)=>b[1]-a[1]).slice(0,8);
    const rentalRows=[{label:'Items still with customer',value:active,color:'#D39734'},{label:'Some items returned',value:partial,color:'#678899'},{label:'All items returned',value:completed,color:'#4B9B67'}];
    const payRows=[{label:'Fully paid',value:paid,color:'#4B9B67'},{label:'Partly paid',value:partPaid,color:'#D39734'},{label:'Payment pending',value:pendingPay,color:'#C95B55'}];
    el.innerHTML=`
      <div class="report-explain">These reports use simple words. The number beside each label is the number of rental transactions.</div>
      <div class="stat-row"><div class="stat"><div class="n">${rentals.length}</div><div class="l">Total Rentals</div></div><div class="stat success"><div class="n">${money(totalReceived)}</div><div class="l">Money Received</div></div></div>
      <div class="stat-row"><div class="stat warn"><div class="n">${money(outstanding)}</div><div class="l">Money Yet to Receive</div></div><div class="stat"><div class="n">${pendingQty}</div><div class="l">Items Yet to Return</div></div></div>
      <div class="simple-chart-card"><div class="simple-chart-title">Rental Return Status</div><div class="simple-chart-help">Shows whether the customer still has the items or has returned them.</div><div class="simple-chart-canvas"><canvas id="rentalSimpleChart"></canvas></div>${reportLegendHtml(rentalRows)}</div>
      <div class="simple-chart-card"><div class="simple-chart-title">Payment Status</div><div class="simple-chart-help">Shows which bills are paid and which bills still have money pending.</div><div class="simple-chart-canvas"><canvas id="paymentSimpleChart"></canvas></div>${reportLegendHtml(payRows)}</div>
      <div class="section-label">Most Rented Items</div><div class="card">${top.length?top.map(([n,q])=>`<div class="report-row"><span class="l">${escapeHtml(n)}</span><span class="v">${q} qty</span></div>`).join(''):'No rentals in this period'}</div>`;
    if(reportsChartRentals) reportsChartRentals.destroy(); if(reportsChartPayments) reportsChartPayments.destroy();
    reportsChartRentals=makeSimpleDoughnut('rentalSimpleChart',rentalRows.map(x=>x.label),rentalRows.map(x=>x.value),rentalRows.map(x=>x.color));
    reportsChartPayments=makeSimpleDoughnut('paymentSimpleChart',payRows.map(x=>x.label),payRows.map(x=>x.value),payRows.map(x=>x.color));
  }catch(e){el.innerHTML=`<div class="alert danger">${escapeHtml(e.message)}</div>`;} hideLoading();
}

/* ---------- PDF: latest reference header and stacked bottom amount rows ---------- */
function pdfFinanceRowsV5(r){
  const total=Number(r.total_amount||0),discount=Number(r.discount_amount||0),final=Number(r.final_amount??total),received=Number(r.amount_received||0),balance=Math.max(0,final-received);
  const rows=[['Total Amount',`Rs ${Math.round(total)}`,'']];
  if(received>0) rows.push(['Received Amount',`Rs ${Math.round(received)}`,'']);
  if(discount>0){rows.push(['Discount',`Rs ${Math.round(discount)}`,'']);rows.push(['Final Amount',`Rs ${Math.round(final)}`,'final']);}
  if(balance>0) rows.push(['Pending Amount',`Rs ${Math.round(balance)}`,'pending']);
  return rows;
}
function buildPdfTemplate(r,lines,s){
  const finance=pdfFinanceRowsV5(r);
  return `<div class="pdf-sheet-v4">
    <div class="pdf-v4-header">
      <div class="pdf-v4-kpm">KPM</div>
      <div class="pdf-v4-owner"><b>● &nbsp;${escapeHtml(s.owner_name||'K Ponnumani Murugesan')}</b><span>Proprietor</span><span>☎ &nbsp;${escapeHtml(s.owner_mobile||'9940841872')}</span><span>⌖ &nbsp;${escapeHtml(s.address||'105, Arasamara Street, Panpoli - 627807')}</span></div>
    ...
    <div class="pdf-v4-table"><div class="pdf-v4-th"><span>#</span><span>ITEM</span><span class="pdf-v4-num">QTY</span><span class="pdf-v4-num">RATE</span><span class="pdf-v4-num">AMOUNT</span></div>
      ${lines.map((l,i)=>`<div class="pdf-v4-tr"><span>${i+1}</span><span class="item">${escapeHtml(l.item_name)}</span><span class="pdf-v4-num">${l.qty}</span><span class="pdf-v4-num">Rs ${Math.round(l.price)}</span><span class="pdf-v4-num">Rs ${Math.round(l.qty*l.price)}</span></div>`).join('')}
    </div>
    <div class="pdf-v4-spacer"></div>
    <div class="pdf-v5-finance">${finance.map(([l,v,c])=>`<div class="pdf-v5-finance-row ${c}"><span>${l}</span><b>${v}</b></div>`).join('')}</div>
    <div class="pdf-v4-thanks"><span class="hands">♧</span>— &nbsp; Thank you! Vanakkam &nbsp; —</div>
  </div>`;
}

/* ---------- Customer number helper for PDF share ---------- */
async function copyCustomerNumber(mobile){
  const digits=(mobile||'').replace(/\D/g,''); if(!digits){toast('Customer mobile number is not available');return;}
  try{await navigator.clipboard.writeText(digits);toast('Customer number copied: '+digits);}catch(e){
    const ta=document.createElement('textarea');ta.value=digits;document.body.appendChild(ta);ta.select();document.execCommand('copy');ta.remove();toast('Customer number copied: '+digits);
  }
}
function shareCustomerBox(r){
  if(!r.mobile) return `<div class="alert warn">Customer mobile number was not entered for this rental.</div>`;
  return `<div class="share-customer-box"><div><div class="share-customer-label">Saved customer WhatsApp number</div><div class="share-customer-number">${escapeHtml(r.mobile)}</div></div><button class="btn btn-outline" onclick="copyCustomerNumber('${escapeHtml(r.mobile)}')">📋 Copy Number</button></div>`;
}
async function renderDetail(){
  const el=document.getElementById('detailContent'); showLoading();
  try{
    const full=await fetchRentalFull(currentDetailId),s=await getShopSettings(); hideLoading(); const {rental:r,lines,pays}=full,bal=billBalance(r);
    el.innerHTML=`${billScreenHtml(r,lines,s)}${shareCustomerBox(r)}<div class="bill-actions"><button class="btn btn-whatsapp" onclick="shareBillPdf('${r.id}')">📄 Share Bill PDF</button><button class="btn btn-outline" onclick="printBill('${r.id}')">🖨️ ${t('print_bill')}</button></div>
      <div class="section-label">${t('payments')}</div>${bal>0?`<button class="btn btn-brass btn-block" onclick="openPaymentForm('${r.id}',${bal})">💰 ${t('record_payment')}</button>`:''}<button class="btn btn-outline btn-block qr-action" onclick="showPaymentQr()">▣ Show UPI QR</button><button class="btn btn-outline btn-block" style="margin-top:10px" onclick="openDiscountForm('${r.id}',${Number(r.total_amount)},${Number(r.discount_amount||0)})">🏷️ ${t('discount')}</button>${pays.length?`<div class="section-label">${t('payment_history')}</div><div class="card">${pays.map(p=>`<div class="balance-box"><span class="l">${fmtDateTime(p.paid_at)}${p.note?' — '+escapeHtml(p.note):''}</span><span class="v">${money(p.amount)}</span></div>`).join('')}</div>`:''}`;
  }catch(e){hideLoading();el.innerHTML=`<div class="alert danger">${escapeHtml(e.message)}</div>`;}
}

/* Native Android PDF share bridge. Chrome/browser still uses Web Share API. */
function blobToBase64Payload(blob){
  return new Promise((resolve,reject)=>{
    ...
    reader.onerror=()=>reject(reader.error||new Error('Unable to prepare PDF for sharing'));
    reader.readAsDataURL(blob);
  });
}
async function tryAndroidNativePdfShare(blob,filename,title){
  try{
    if(window.AndroidShare && typeof window.AndroidShare.sharePdfBase64==='function'){
      const b64=await blobToBase64Payload(blob);
      window.AndroidShare.sharePdfBase64(b64,filename,title||'KPM PDF');
      return true;
    }
  }catch(e){ console.warn('Android native share failed',e); }
  return false;
}

/* Share-sheet first. The PDF stays attached; customer number is visible/copyable on Bill page. */
async function shareBillPdf(id){
  showLoading('Preparing PDF...');
  try{
    const full=await fetchRentalFull(id),s=await getShopSettings(),blob=await createBillPdfBlob(full.rental,full.lines,s),filename=`${full.rental.bill_no}.pdf`,file=new File([blob],filename,{type:'application/pdf'});
    hideLoading();

    // APK/WebView: use native Android FileProvider share. PDF is attached.
    if(await tryAndroidNativePdfShare(blob,filename,`KPM ${full.rental.bill_no}`)) return;

    // Chrome/normal browser: use the Web Share API when file sharing is supported.
    if(navigator.share && (!navigator.canShare || navigator.canShare({files:[file]}))){
      try{
        await navigator.share({title:`KPM ${full.rental.bill_no}`,text:full.rental.mobile?`Customer: ${full.rental.customer_name} | Mobile: ${full.rental.mobile}`:`Customer: ${full.rental.customer_name}`,files:[file]});
        return;
      }catch(e){ if(e?.name==='AbortError') return; console.warn(e); }
    }

    downloadPdfBlob(blob,filename);
    if(full.rental.mobile) await copyCustomerNumber(full.rental.mobile);
    toast('PDF downloaded. Customer number has been copied.');
  }catch(e){hideLoading();if(e?.name!=='AbortError')alert(e.message);}
}
...
async function sharePendingPdf(r,pending){
  showLoading('Preparing pending PDF...');
  try{
    const s=await getShopSettings(),blob=await createPendingPdfBlob(r,pending,s),filename=`${r.bill_no}-PENDING.pdf`,file=new File([blob],filename,{type:'application/pdf'});
    hideLoading();

    if(await tryAndroidNativePdfShare(blob,filename,`KPM Pending Items ${r.bill_no}`)) return;

    if(navigator.share && (!navigator.canShare || navigator.canShare({files:[file]}))){
      try{
        await navigator.share({title:`KPM Pending Items ${r.bill_no}`,text:r.mobile?`Customer: ${r.customer_name} | Mobile: ${r.mobile}`:`Customer: ${r.customer_name}`,files:[file]});
        return;
      }catch(e){if(e?.name==='AbortError')return;console.warn(e);}
    }
    downloadPdfBlob(blob,filename);
    if(r.mobile) await copyCustomerNumber(r.mobile);
    toast('Pending PDF downloaded. Customer number copied.');
  }catch(e){hideLoading();if(e?.name!=='AbortError')alert(e.message);}
}

/* =========================================================
   KPM FRONTEND V7 OVERRIDES
   - Conditional Pending Qty columns on partially-returned bills
   - Payment + QR on one row
   - No duplicate Pending Share button on Return Detail
   - Partial rentals available in Active Rental Edit
   - Pending PDF shows ALL items + payment summary
========================================================= */

function kpmHasPartialPending(r, lines){
  const hasReturned=(lines||[]).some(l=>Number(l.received_qty||0)>0);
  const hasPending=(lines||[]).some(l=>Number(l.qty||0)>Number(l.received_qty||0));
  return r?.rental_status==='PARTIALLY_RETURNED' || (hasReturned && hasPending);
}
function kpmPendingQty(line){ return Math.max(0, Number(line.qty||0)-Number(line.received_qty||0)); }
function kpmPaymentQrActions(r){
  const bal=billBalance(r);
  return `<div style="display:flex;gap:10px;margin-bottom:10px;align-items:stretch;">
    ${bal>0?`<button class="btn btn-brass" style="flex:1;min-width:0" onclick="openPaymentForm('${r.id}',${bal})">💰 ${t('record_payment')}</button>`:''}
    <button class="btn btn-outline qr-action" style="flex:${bal>0?'1':'1 1 100%'};min-width:0" onclick="showPaymentQr()">▣ Show UPI QR</button>
  </div>`;
}

/* Bill shown everywhere in the app. Pending Qty appears ONLY for a partially-returned transaction. */
function billScreenHtml(r,lines,s){
  const showPending=kpmHasPartialPending(r,lines);
  return `<div class="card bill-v3" id="billCard">
    <div class="bill-simple-head">
      <div class="bill-simple-kpm">KPM</div>
      <div class="bill-simple-no">${escapeHtml(r.bill_no)}</div>
      <div class="bill-simple-owner"><b>${escapeHtml(s.owner_name||'K Ponnumani Murugesan')}</b><br>${escapeHtml(s.owner_mobile||'9940841872')}<br>${escapeHtml(s.address||'105, Arasamara Street, Panpoli - 627807')}</div>
      <div class="divider"></div>
      <div class="bill-simple-customer"><b>${escapeHtml(r.customer_name)}</b>${r.mobile?`<br>${escapeHtml(r.mobile)}`:''}${r.address?`<br>${escapeHtml(r.address)}`:''}<br>${fmtDateTime(r.rental_date)}</div>
    </div>
    <div class="section-label" style="margin:16px 0 8px">Rental Items</div>
    <div class="bill-v3-table-head" style="grid-template-columns:${showPending?'1.8fr .55fr .75fr .8fr':'1.8fr .65fr .85fr'}">
      <span>PRODUCT</span><span class="num">QTY</span>${showPending?'<span class="num">PENDING</span>':''}<span class="num">AMOUNT</span>
    </div>
    ${(lines||[]).map(l=>{const p=kpmPendingQty(l);return `<div class="bill-v3-row" style="grid-template-columns:${showPending?'1.8fr .55fr .75fr .8fr':'1.8fr .65fr .85fr'}"><div><b>${escapeHtml(l.item_name)}</b><div class="rate">${money(l.price)} each</div></div><div class="num"><b>${l.qty}</b></div>${showPending?`<div class="num"><b style="color:${p>0?'var(--danger)':'var(--success)'}">${p}</b></div>`:''}<div class="num"><b>${money(l.qty*l.price)}</b></div></div>`;}).join('')}
    <div class="bill-v3-finance">${conditionalFinanceHtml(r)}</div>
  </div>`;
}

/* Normal bill PDF. If transaction is partial, add a Pending Qty column. */
function buildPdfTemplate(r,lines,s){
  const finance=pdfFinanceRowsV5(r), showPending=kpmHasPartialPending(r,lines);
  return `<div class="pdf-sheet-v4">
    <div class="pdf-v4-header">
      <div class="pdf-v4-kpm">KPM</div>
      <div class="pdf-v4-owner"><b>● &nbsp;${escapeHtml(s.owner_name||'K Ponnumani Murugesan')}</b><span>Proprietor</span><span>☎ &nbsp;${escapeHtml(s.owner_mobile||'9940841872')}</span><span>⌖ &nbsp;${escapeHtml(s.address||'105, Arasamara Street, Panpoli - 627807')}</span></div>
    ...
    <div class="pdf-v4-table">
      <div class="pdf-v4-th" style="grid-template-columns:${showPending?'40px 1fr 90px 100px 100px 115px':'40px 1fr 100px 100px 115px'}"><span>#</span><span>ITEM</span><span class="pdf-v4-num">QTY</span>${showPending?'<span class="pdf-v4-num">PENDING</span>':''}<span class="pdf-v4-num">RATE</span><span class="pdf-v4-num">AMOUNT</span></div>
      ${(lines||[]).map((l,i)=>{const p=kpmPendingQty(l);return `<div class="pdf-v4-tr" style="grid-template-columns:${showPending?'40px 1fr 90px 100px 100px 115px':'40px 1fr 100px 100px 115px'}"><span>${i+1}</span><span class="item">${escapeHtml(l.item_name)}</span><span class="pdf-v4-num">${l.qty}</span>${showPending?`<span class="pdf-v4-num" style="font-weight:800;color:${p>0?'#B45309':'#2F855A'}">${p}</span>`:''}<span class="pdf-v4-num">Rs ${Math.round(l.price)}</span><span class="pdf-v4-num">Rs ${Math.round(l.qty*l.price)}</span></div>`;}).join('')}
    </div>
    <div class="pdf-v4-spacer"></div>
    <div class="pdf-v5-finance">${finance.map(([l,v,c])=>`<div class="pdf-v5-finance-row ${c}"><span>${l}</span><b>${v}</b></div>`).join('')}</div>
    <div class="pdf-v4-thanks"><span class="hands">♧</span>— &nbsp; Thank you! Vanakkam &nbsp; —</div>
...
/* Payment and QR are side-by-side on Bill page. */
async function renderDetail(){
  const el=document.getElementById('detailContent'); showLoading();
  try{
    const full=await fetchRentalFull(currentDetailId),s=await getShopSettings(); hideLoading(); const {rental:r,lines,pays}=full;
    el.innerHTML=`${billScreenHtml(r,lines,s)}${shareCustomerBox(r)}<div class="bill-actions"><button class="btn btn-whatsapp" onclick="shareBillPdf('${r.id}')">📄 Share Bill PDF</button><button class="btn btn-outline" onclick="printBill('${r.id}')">🖨️ ${t('print_bill')}</button></div>
      <div class="section-label">${t('payments')}</div>${kpmPaymentQrActions(r)}<button class="btn btn-outline btn-block" style="margin-top:0" onclick="openDiscountForm('${r.id}',${Number(r.total_amount)},${Number(r.discount_amount||0)})">🏷️ ${t('discount')}</button>${pays.length?`<div class="section-label">${t('payment_history')}</div><div class="card">${pays.map(p=>`<div class="balance-box"><span class="l">${fmtDateTime(p.paid_at)}${p.note?' — '+escapeHtml(p.note):''}</span><span class="v">${money(p.amount)}</span></div>`).join('')}</div>`:''}`;
  }catch(e){hideLoading();el.innerHTML=`<div class="alert danger">${escapeHtml(e.message)}</div>`;}
}

/* Return detail: remove duplicate Pending Share. Payment + QR on one row. */
async function renderReturnDetail(){
  const el=document.getElementById('returnDetailContent');showLoading();
  try{
    const full=await fetchRentalFull(currentDetailId),s=await getShopSettings(),r=full.rental;hideLoading();
    el.innerHTML=`${returnBillPreviewHtml(r,full.lines,s)}<div class="section-label">Return Items</div><div class="card" id="returnLinesCard">${full.lines.map((l,i)=>{const cur=kpmPendingQty(l);return `<div class="return-item-line"><div class="return-item-name">${escapeHtml(l.item_name)}</div><div class="return-inline"><div class="return-actual">Qty <b>${l.qty}</b>${l.received_qty?`<div class="sub">Returned ${l.received_qty}</div>`:''}</div><div class="return-pending-wrap"><label>Pending</label><input id="pending-${i}" data-line="${l.id}" data-total="${l.qty}" data-received="${l.received_qty}" data-last-pending="${cur}" type="number" inputmode="numeric" min="0" max="${cur}" value="${cur}" onfocus="this.select()" onclick="this.select()" oninput="syncReturnChoice(${i})"></div><button id="allrecv-${i}" class="btn btn-outline return-all-btn ${cur===0?'selected':''}" onclick="setReturnAll(${i})">✓ All</button></div></div>`;}).join('')}</div>
      <div class="section-label">${t('payments')}</div><div class="card">${conditionalFinanceHtml(r,true)}</div>${kpmPaymentQrActions(r)}<button class="btn btn-success btn-block" id="confirmReturnBtn" onclick="submitReturnV2()">${t('confirm_return')}</button>`;
  }catch(e){hideLoading();el.innerHTML=`<div class="alert danger">${escapeHtml(e.message)}</div>`;}
}

/* Active Rental Edit now lists ACTIVE + PARTIALLY_RETURNED transactions. */
async function renderEditRentals(){
  showLoading();const el=document.getElementById('editRentalsList');
  try{
    const {data,error}=await sb.from('rentals').select('*,rental_items(qty,received_qty)').in('rental_status',['ACTIVE','PARTIALLY_RETURNED']).order('rental_date',{ascending:false});
    if(error)throw error;let list=data||[];const q=(document.getElementById('editRentalSearch').value||'').toLowerCase();if(q)list=list.filter(r=>(r.customer_name||'').toLowerCase().includes(q)||(r.mobile||'').includes(q)||(r.bill_no||'').toLowerCase().includes(q));
    el.innerHTML=list.length?list.map(r=>renderRentalCard(r,`openEditRental('${r.id}')`)).join(''):emptyState('✏️','No active or partially-returned rentals','');
  }catch(e){el.innerHTML=`<div class="alert danger">${escapeHtml(e.message)}</div>`;}hideLoading();
}

/* Partial-edit UI: never allow quantity below quantity already returned. */
function setEditQty(id,val){
  const item=ITEMS_CACHE.find(x=>x.id===id); if(!item)return;
  const line=editingRentalFull?.lines.find(l=>l.item_id===id),min=Number(line?.received_qty||0),max=editAvailable(item);
  let q=parseInt(val); if(isNaN(q))q=min; q=Math.max(min,Math.min(q,max)); editRentalSelections[id]=q; renderEditRentalDetail(false);
}
function removeEditRentalItem(id){
  const it=ITEMS_CACHE.find(x=>x.id===id);if(!it)return;const line=editingRentalFull?.lines.find(l=>l.item_id===id);
  if(Number(line?.received_qty||0)>0){toast(`Cannot remove ${it.name}: ${line.received_qty} qty already returned. You can reduce only to ${line.received_qty}.`);return;}
  if(!confirm(`Remove ${it.name} from this bill?`))return;editRentalSelections[id]=0;renderEditRentalDetail(false);
}
function renderEditRentalDetail(scroll=true){
  const el=document.getElementById('editRentalContent');if(!editingRentalFull){el.innerHTML=emptyState('✏️','No rental loaded','');return;}
  const r=editingRentalFull.rental,ids=currentEditItemIds(),selectedItems=ids.map(id=>ITEMS_CACHE.find(x=>x.id===id)).filter(Boolean),selectedTotal=selectedItems.reduce((s,it)=>s+(editRentalSelections[it.id]||0)*Number(it.price),0);
  el.innerHTML=`<div class="card"><div class="customer-highlight">${escapeHtml(r.customer_name)}</div><div class="customer-sub">${escapeHtml(r.bill_no)} • ${fmtDateTime(r.rental_date)}<br>Received Payment: ${money(r.amount_received)}${r.rental_status==='PARTIALLY_RETURNED'?'<br><b>Partially Returned</b>':''}</div></div><div class="section-label">Bill Items</div><div class="edit-bill-items">${selectedItems.map(it=>{const q=editRentalSelections[it.id]||0,max=editAvailable(it),line=editingRentalFull.lines.find(l=>l.item_id===it.id),min=Number(line?.received_qty||0);return `<div class="edit-item-row"><div class="edit-item-top"><div class="item-thumb">${it.photo_url?`<img src="${it.photo_url}">`:'🍽️'}</div><div class="item-info"><div class="nm">${escapeHtml(it.name)}</div>${it.name_en?`<div class="nmta">${escapeHtml(it.name_en)}</div>`:''}<div class="sub">${money(it.price)} • Available: ${max}${min>0?` • Already returned: ${min}`:''}</div></div></div><div class="edit-item-actions"><button onclick="adjustEditQty('${it.id}',-1)">−</button><input type="number" min="${min}" max="${max}" value="${q}" onchange="setEditQty('${it.id}',this.value)"><button onclick="adjustEditQty('${it.id}',1)">+</button><b style="margin-left:auto">${money(q*it.price)}</b><button class="edit-delete" onclick="removeEditRentalItem('${it.id}')">🗑</button></div></div>`;}).join('')}</div><button class="btn btn-outline btn-block add-rental-item-btn" onclick="openAddRentalItem()">➕ Add New Item</button><div class="total-box"><div class="lbl">Updated Total Before Discount</div><div class="val">${money(selectedTotal)}</div></div><button class="btn btn-brass btn-block" id="saveEditRentalBtn" onclick="saveEditedRental()">${t('save_updated_bill')}</button>`;if(scroll)window.scrollTo(0,0);
}

/* Pending PDF: show ALL items. 0-pending rows first, pending rows last/highlighted. Include payment summary. */
function buildPendingPdfTemplate(r,rows,s){
  const ordered=[...(rows||[])].sort((a,b)=>Number(a.pending||0)-Number(b.pending||0));
  const finance=pdfFinanceRowsV5(r);
  return `<div class="pdf-sheet-v4">
    <div class="pdf-v4-header"><div class="pdf-v4-kpm">KPM</div><div class="pdf-v4-owner"><b>● &nbsp;${escapeHtml(s.owner_name||'K Ponnumani Murugesan')}</b><span>Proprietor</span><span>☎ &nbsp;${escapeHtml(s.owner_mobile||'9940841872')}</span><span>⌖ &nbsp;${escapeHtml(s.address||'105, Arasamara Street, Panpoli - 627807')}</span></div><div class="pdf-v4-meta"><b>▣ &nbsp;Bill No.</b> &nbsp;: ${escapeHtml(r.bill_no)}<br><b>▦ &nbsp;Date & Time</b> &nbsp;: ${fmtDateTime(r.rental_date)}<br><b>● &nbsp;Customer</b> &nbsp;: <b>${escapeHtml(r.customer_name)}</b>${r.mobile?`<br><b>☎ &nbsp;Mobile</b> &nbsp;: ${escapeHtml(r.mobile)}`:''}${r.address?`<br><b>⌖ &nbsp;Address</b> &nbsp;: ${escapeHtml(r.address)}`:''}</div><img class="pdf-v4-amman" src="assets/amman-white.jpg" alt="Amman"></div>
    <div class="pdf-v4-table"><div class="pdf-v4-pending-th"><span>#</span><span>ITEM</span><span class="pdf-v4-num">TOTAL QTY</span><span class="pdf-v4-num">PENDING QTY</span></div>${ordered.map((l,i)=>`<div class="pdf-v4-pending-tr"><span>${i+1}</span><span class="item"><b>${escapeHtml(l.name||l.item_name)}</b></span><span class="pdf-v4-num">${l.total??l.qty}</span><span class="pdf-v4-num"><b style="color:${Number(l.pending)>0?'#B45309':'#2F855A'}">${Number(l.pending||0)}</b></span></div>`).join('')}</div>
    <div class="pdf-v4-spacer"></div>
    <div class="pdf-v5-finance">${finance.map(([l,v,c])=>`<div class="pdf-v5-finance-row ${c}"><span>${l}</span><b>${v}</b></div>`).join('')}</div>
    <div class="pdf-v4-thanks">— &nbsp; Thank you! Vanakkam &nbsp; —</div>
  </div>`;
}
async function sharePendingPreview(){
  try{
    const full=await fetchRentalFull(currentDetailId),inputs=[...document.querySelectorAll('#returnLinesCard input[data-line]')];
    const rows=full.lines.map(line=>{const inp=inputs.find(x=>x.dataset.line===line.id);const p=inp?Math.max(0,parseInt(inp.value)||0):kpmPendingQty(line);return {name:line.item_name,total:line.qty,pending:p};});
    if(!rows.some(x=>x.pending>0)){toast('No pending items to share');return;} await sharePendingPdf(full.rental,rows);
  }catch(e){alert(e.message);}
}
async function sharePendingWhatsapp(rentalId){
  try{
    const full=await fetchRentalFull(rentalId),rows=full.lines.map(l=>({name:l.item_name,total:l.qty,pending:kpmPendingQty(l)}));
    if(!rows.some(x=>x.pending>0)){toast('No pending items to share');return;} await sharePendingPdf(full.rental,rows);
  }catch(e){alert(e.message);}
}
...
/* =========================================================
   KPM FRONTEND V8 — usability + ordering + admin cleanup
   PDF template intentionally unchanged.
========================================================= */

/* ---------- V8 styles ---------- */
(function injectKpmV8Styles(){
  if(document.getElementById('kpm-v8-style')) return;
  const st=document.createElement('style');
  st.id='kpm-v8-style';
  st.textContent=`
    .item-card.kpm-sortable{position:relative;touch-action:pan-y;user-select:none;-webkit-user-select:none;transition:transform .16s ease,box-shadow .16s ease,opacity .16s ease}
    .item-card.kpm-sortable.kpm-pressing{transform:scale(.992);box-shadow:0 7px 22px rgba(0,0,0,.10)}
    .item-card.kpm-dragging{opacity:.18!important}
    .kpm-drag-ghost{position:fixed!important;z-index:99999!important;pointer-events:none!important;margin:0!important;box-sizing:border-box!important;opacity:.96!important;box-shadow:0 18px 46px rgba(0,0,0,.24)!important;transform:scale(1.015);transition:none!important}
    .kpm-sort-placeholder{border:2px dashed rgba(185,132,43,.5)!important;background:rgba(245,234,215,.45)!important;box-shadow:none!important}
    .kpm-sort-placeholder>*{visibility:hidden!important}
    .item-order-help{font-size:13px;color:var(--muted);margin:0 0 10px 3px;font-weight:650}
    .bill-old-head{text-align:center;padding:2px 0 4px}
    .bill-old-no{font-size:18px;color:var(--muted);font-weight:650;margin-bottom:24px}
    .bill-old-owner{font-size:17px;line-height:1.7;color:var(--ink)}
    .bill-old-owner b{font-size:19px}
    .bill-old-divider{height:1px;background:var(--line);margin:22px 0}
    .bill-old-customer{text-align:left}
    .bill-old-customer .name{font-size:24px;font-weight:850;color:var(--ink);margin-bottom:8px}
    .bill-old-customer .meta{font-size:17px;line-height:1.75;color:var(--muted)}
    .bill-old-items-title{font-size:16px;font-weight:850;color:var(--muted);letter-spacing:.04em;margin:18px 0 10px}
    .bill-old-grid-head,.bill-old-grid-row{display:grid;align-items:center;column-gap:8px}
    .bill-old-grid-head{padding:10px 6px;border-bottom:1px solid var(--line);font-size:14px;font-weight:850;color:var(--muted)}
    .bill-old-grid-row{padding:13px 6px;border-bottom:1px solid var(--line);font-size:17px}
    .bill-old-grid-row .item-name{font-size:20px;font-weight:850}
    .bill-old-grid-row .rate{font-size:14px;margin-top:2px;color:var(--ink-soft)}
    .bill-old-grid-row .num,.bill-old-grid-head .num{text-align:right}
    .bill-old-finance{margin-top:14px}
    .delete-transaction-btn{font-size:13px!important;padding:8px 12px!important;min-height:38px!important;width:auto!important;margin:10px 0 0 auto!important;display:flex!important}
    .font-setting-box{margin-top:18px;padding-top:16px;border-top:1px solid var(--line)}
    .font-setting-title{font-size:16px;font-weight:850;margin-bottom:5px}
    .font-setting-help{font-size:13px;color:var(--muted);margin-bottom:10px}
    .font-setting-actions{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}
    .font-setting-actions button.selected{background:var(--brass);color:#fff;border-color:var(--brass)}
    body.kpm-font-small button,body.kpm-font-small input,body.kpm-font-small textarea,body.kpm-font-small select,body.kpm-font-small .nm,body.kpm-font-small .customer-highlight,body.kpm-font-small .customer-sub,body.kpm-font-small .section-label,body.kpm-font-small .bill-old-grid-row,body.kpm-font-small .bill-old-customer .meta{font-size:92%!important}
    body.kpm-font-large button,body.kpm-font-large input,body.kpm-font-large textarea,body.kpm-font-large select,body.kpm-font-large .nm,body.kpm-font-large .customer-highlight,body.kpm-font-large .customer-sub,body.kpm-font-large .section-label,body.kpm-font-large .bill-old-grid-row,body.kpm-font-large .bill-old-customer .meta{font-size:112%!important}
  `;
  document.head.appendChild(st);
})();

/* ---------- Persistent item order ---------- */
async function loadItemsAndStock(includeInactive=false){
  let q=sb.from('items').select('*').order('sort_order',{ascending:true,nullsFirst:false}).order('name',{ascending:true});
  const {data:items,error}=await q;
  if(error) throw error;
  ITEMS_CACHE=includeInactive?(items||[]):(items||[]).filter(i=>i.active);
  const {data:lines,error:e2}=await sb.from('rental_items').select('item_id, qty, received_qty, rentals!inner(rental_status)').neq('rentals.rental_status','COMPLETED');
  if(e2) throw e2;
  OUTSTANDING_CACHE={};
  (lines||[]).forEach(li=>{OUTSTANDING_CACHE[li.item_id]=(OUTSTANDING_CACHE[li.item_id]||0)+Math.max(0,li.qty-li.received_qty);});
  return items||[];
}

async function saveCurrentItemOrder(){
  if(currentProfile?.role!=='admin') return;
  const ids=[...document.querySelectorAll('#itemsList .item-card[data-item-id]')].map(x=>x.dataset.itemId);
  try{
    const results=await Promise.all(ids.map((id,i)=>sb.from('items').update({sort_order:i+1}).eq('id',id)));
    const bad=results.find(x=>x.error); if(bad?.error) throw bad.error;
    ids.forEach((id,i)=>{const item=ITEMS_CACHE.find(x=>x.id===id);if(item)item.sort_order=i+1;});
    toast('Item order saved');
  }catch(e){alert('Unable to save item order: '+(e.message||e));}
}

let kpmDragCard=null;
let kpmDragGhost=null;
let kpmDragPlaceholder=null;
let kpmDragPointerId=null;
let kpmDragOffsetY=0;
let kpmDragOffsetX=0;
let kpmDragPressTimer=null;
let kpmDragStartX=0;
let kpmDragStartY=0;
let kpmDragAutoScrollRaf=null;
let kpmDragAutoScrollSpeed=0;

function kpmStopAutoScroll(){
  kpmDragAutoScrollSpeed=0;
  if(kpmDragAutoScrollRaf){cancelAnimationFrame(kpmDragAutoScrollRaf);kpmDragAutoScrollRaf=null;}
}
function kpmRunAutoScroll(){
  if(!kpmDragCard||!kpmDragAutoScrollSpeed){kpmDragAutoScrollRaf=null;return;}
  window.scrollBy(0,kpmDragAutoScrollSpeed);
  kpmDragAutoScrollRaf=requestAnimationFrame(kpmRunAutoScroll);
}
function kpmSetAutoScroll(clientY){
  const edge=Math.min(105,Math.max(70,window.innerHeight*.14));
  let speed=0;
  if(clientY<edge) speed=-Math.max(3,Math.round((edge-clientY)/6));
  else if(clientY>window.innerHeight-edge) speed=Math.max(3,Math.round((clientY-(window.innerHeight-edge))/6));
  speed=Math.max(-16,Math.min(16,speed));
  kpmDragAutoScrollSpeed=speed;
  if(speed&&!kpmDragAutoScrollRaf) kpmDragAutoScrollRaf=requestAnimationFrame(kpmRunAutoScroll);
  if(!speed) kpmStopAutoScroll();
}
function kpmBeginCardDrag(card,ev){
  const list=document.getElementById('itemsList');if(!list||!card)return;
  const r=card.getBoundingClientRect();
  kpmDragCard=card;kpmDragPointerId=ev.pointerId;
  kpmDragOffsetY=ev.clientY-r.top;kpmDragOffsetX=ev.clientX-r.left;
  card.classList.remove('kpm-pressing');card.classList.add('kpm-dragging');

  kpmDragPlaceholder=card.cloneNode(true);
  kpmDragPlaceholder.classList.remove('kpm-dragging','kpm-pressing');
  kpmDragPlaceholder.classList.add('kpm-sort-placeholder');
  kpmDragPlaceholder.removeAttribute('data-item-id');
  card.parentNode.insertBefore(kpmDragPlaceholder,card.nextSibling);

  kpmDragGhost=card.cloneNode(true);
  kpmDragGhost.classList.remove('kpm-dragging','kpm-pressing');
  kpmDragGhost.classList.add('kpm-drag-ghost');
  kpmDragGhost.style.width=r.width+'px';
  kpmDragGhost.style.height=r.height+'px';
  document.body.appendChild(kpmDragGhost);
  kpmMoveGhost(ev.clientX,ev.clientY);
  try{card.setPointerCapture(ev.pointerId);}catch(_){ }
  if(navigator.vibrate) try{navigator.vibrate(18);}catch(_){ }
}
function kpmMoveGhost(x,y){
  if(!kpmDragGhost)return;
  const maxX=Math.max(4,window.innerWidth-kpmDragGhost.offsetWidth-4);
  const left=Math.max(4,Math.min(maxX,x-kpmDragOffsetX));
  kpmDragGhost.style.left=left+'px';
  kpmDragGhost.style.top=(y-kpmDragOffsetY)+'px';
}
function kpmMovePlaceholder(clientX,clientY){
  if(!kpmDragCard||!kpmDragPlaceholder)return;
  const list=document.getElementById('itemsList');if(!list)return;
  const cards=[...list.querySelectorAll('.item-card.kpm-sortable:not(.kpm-dragging):not(.kpm-sort-placeholder)')];
  let target=null,after=false;
  for(const c of cards){
    const r=c.getBoundingClientRect();
    if(clientY>=r.top-8&&clientY<=r.bottom+8){target=c;after=clientY>r.top+r.height/2;break;}
  }
  if(target){
    if(after) list.insertBefore(kpmDragPlaceholder,target.nextSibling);
    else list.insertBefore(kpmDragPlaceholder,target);
  }else if(cards.length){
    const first=cards[0].getBoundingClientRect(),last=cards[cards.length-1].getBoundingClientRect();
    if(clientY<first.top) list.insertBefore(kpmDragPlaceholder,cards[0]);
    else if(clientY>last.bottom) list.appendChild(kpmDragPlaceholder);
  }
}
async function kpmFinishCardDrag(card,ev,cancelled=false){
  clearTimeout(kpmDragPressTimer);kpmDragPressTimer=null;kpmStopAutoScroll();
  card?.classList.remove('kpm-pressing');
  if(!kpmDragCard)return;
  const list=document.getElementById('itemsList');
  try{if(kpmDragPointerId!=null)kpmDragCard.releasePointerCapture(kpmDragPointerId);}catch(_){ }
  if(!cancelled&&list&&kpmDragPlaceholder) list.insertBefore(kpmDragCard,kpmDragPlaceholder);
  kpmDragCard.classList.remove('kpm-dragging');
  kpmDragGhost?.remove();kpmDragPlaceholder?.remove();
  kpmDragGhost=null;kpmDragPlaceholder=null;kpmDragCard=null;kpmDragPointerId=null;
  if(!cancelled) await saveCurrentItemOrder();
}
function initKpmItemSorting(){
  if(currentProfile?.role!=='admin') return;
  const list=document.getElementById('itemsList');if(!list)return;
  list.querySelectorAll('.item-card.kpm-sortable').forEach(card=>{
    card.onpointerdown=(ev)=>{
      if(ev.pointerType==='mouse'&&ev.button!==0)return;
      if(ev.target.closest('button,a,input,select,textarea'))return;
      kpmDragStartX=ev.clientX;kpmDragStartY=ev.clientY;
      card.classList.add('kpm-pressing');
      clearTimeout(kpmDragPressTimer);
      kpmDragPressTimer=setTimeout(()=>kpmBeginCardDrag(card,ev),180);
    };
    card.onpointermove=(ev)=>{
      if(kpmDragCard===card){
        ev.preventDefault();
        kpmMoveGhost(ev.clientX,ev.clientY);
        kpmMovePlaceholder(ev.clientX,ev.clientY);
        kpmSetAutoScroll(ev.clientY);
        return;
      }
      if(Math.abs(ev.clientX-kpmDragStartX)>8||Math.abs(ev.clientY-kpmDragStartY)>8){
        clearTimeout(kpmDragPressTimer);kpmDragPressTimer=null;card.classList.remove('kpm-pressing');
      }
    };
    card.onpointerup=(ev)=>kpmFinishCardDrag(card,ev,false);
    card.onpointercancel=(ev)=>kpmFinishCardDrag(card,ev,true);
    card.oncontextmenu=(ev)=>{if(kpmDragCard===card||card.classList.contains('kpm-pressing'))ev.preventDefault();};
  });
}
...
async function renderItems(){
  const isAdmin=currentProfile?.role==='admin';
  const addBtn=document.getElementById('addItemBtn');if(addBtn)addBtn.style.display=isAdmin?'flex':'none';
  showLoading();try{await loadItemsAndStock(true);}catch(e){hideLoading();toast(e.message);return;}hideLoading();
  const list=document.getElementById('itemsList');
  if(!ITEMS_CACHE.length){list.innerHTML=emptyState('🍽️','No items yet','Add your first rental item');return;}
  list.innerHTML=(isAdmin?`<div class="item-order-help">Hold any empty area of an item card briefly, then drag it up or down to change the order.</div>`:'')+ITEMS_CACHE.map(item=>{
    const rented=OUTSTANDING_CACHE[item.id]||0,avail=availableQty(item),thumb=item.photo_url?`<img src="${item.photo_url}">`:'🍽️';
    return `<div class="item-card kpm-sortable" data-item-id="${item.id}"><div class="item-thumb">${thumb}</div><div class="item-info"><div class="nm">${escapeHtml(item.name)}</div>${item.name_en?`<div class="nmta">${escapeHtml(item.name_en)}</div>`:''}<div class="sub">Rate: ${money(item.price)} / unit</div><div class="stockline"><span class="tot">Total ${item.total_qty}</span>${item.active?`<span class="${avail<=0?'zero':'avail'}">Available ${avail}</span>`:`<span class="inactive">Inactive</span>`}${rented?`<span class="rented">Rented ${rented}</span>`:''}</div></div>${isAdmin?`<div style="display:flex;flex-direction:column;gap:7px"><button class="btn btn-outline btn-sm" onclick="openItemForm('${item.id}')">${t('edit')}</button><button class="btn ${item.active?'btn-danger-outline':'btn-outline'} btn-sm" onclick="toggleItemActive('${item.id}',${!item.active})">${item.active?t('deactivate'):t('activate')}</button></div>`:''}</div>`;
  }).join('');
  requestAnimationFrame(initKpmItemSorting);
}

/* ---------- One-tap quantity replacement in New Rental ---------- */
function renderPickItems(){
  const list=document.getElementById('pickItemsList');if(!ITEMS_CACHE.length){list.innerHTML=emptyState('🍽️','No items available','Add items first');return;}
  list.innerHTML=ITEMS_CACHE.map(item=>{const avail=availableQty(item),qty=newRentalSelections[item.id]||0,thumb=item.photo_url?`<img src="${item.photo_url}">`:'🍽️';return `<div class="pick-card ${qty>0?'selected':''}"><div class="pick-top"><div class="item-thumb">${thumb}</div><div class="item-info"><div class="nm">${escapeHtml(item.name)}</div>${item.name_en?`<div class="nmta">${escapeHtml(item.name_en)}</div>`:''}<div class="price">Rate ${money(item.price)}</div><div class="avail-tag ${avail<=0?'low':''}">Available: ${avail}</div></div></div><div class="pick-bottom"><div class="stepper"><button onclick="adjustPick('${item.id}',-1)" ${qty<=0?'disabled':''}>−</button><input class="qty-direct num" type="number" inputmode="numeric" min="0" max="${avail}" value="${qty}" onfocus="this.select()" onclick="this.select()" onchange="setPickQty('${item.id}',this.value)"><button onclick="adjustPick('${item.id}',1)" ${qty>=avail?'disabled':''}>+</button></div><span class="lineamt num">${money(qty*item.price)}</span></div></div>`;}).join('');
}

/* ---------- Restore the older in-app Bill appearance ---------- */
function billScreenHtml(r,lines,s){
  const showPending=kpmHasPartialPending(r,lines);
  const cols=showPending?'1.7fr .52fr .7fr .82fr':'1.8fr .65fr .9fr';
  return `<div class="card" id="billCard">
    <div class="bill-old-head"><div class="bill-old-no">${escapeHtml(r.bill_no)}</div><div class="bill-old-owner"><b>${escapeHtml(s.owner_name||'K Ponnumani Murugesan')}</b><br>${escapeHtml(s.owner_mobile||'9940841872')}<br>${escapeHtml(s.address||'105, Arasamara Street, Panpoli - 627807')}</div></div>
    <div class="bill-old-divider"></div>
    <div class="bill-old-customer"><div class="name">${escapeHtml(r.customer_name)}</div><div class="meta">${r.mobile?escapeHtml(r.mobile)+'<br>':''}${r.address?escapeHtml(r.address)+'<br>':''}${fmtDateTime(r.rental_date)}</div></div>
    <div class="bill-old-divider"></div>
    <div class="bill-old-items-title">RENTAL ITEMS</div>
    <div class="bill-old-grid-head" style="grid-template-columns:${cols}"><span>PRODUCT</span><span class="num">QTY</span>${showPending?'<span class="num">PENDING</span>':''}<span class="num">AMOUNT</span></div>
    ${(lines||[]).map(l=>{const p=kpmPendingQty(l);return `<div class="bill-old-grid-row" style="grid-template-columns:${cols}"><div><div class="item-name">${escapeHtml(l.item_name)}</div><div class="rate">${money(l.price)} each</div></div><div class="num"><b>${l.qty}</b></div>${showPending?`<div class="num"><b style="color:${p>0?'var(--danger)':'var(--success)'}">${p}</b></div>`:''}<div class="num"><b>${money(l.qty*l.price)}</b></div></div>`;}).join('')}
    <div class="bill-old-finance">${conditionalFinanceHtml(r)}</div>
  </div>`;
}

/* ---------- Admin-only transaction deletion ---------- */
async function deleteTransactionAdmin(rentalId,billNo){
  if(currentProfile?.role!=='admin'){toast('Admin access required');return;}
  if(!confirm(`Delete transaction ${billNo}?\n\nThis permanently deletes the bill, payments, returns and edit history for this transaction.`))return;
  if(!confirm('This cannot be undone. Delete this transaction permanently?'))return;
  showLoading();
  try{
    const {error}=await sb.rpc('delete_rental_admin',{p_rental_id:rentalId});
    if(error)throw error;
    hideLoading();toast('Transaction deleted');
    currentDetailId=null;navStack=['dashboard'];nav('history',false);
  }catch(e){hideLoading();alert(e.message||e);}
}
...
async function renderDetail(){
  ...
  try{
    const full=await fetchRentalFull(currentDetailId),s=await getShopSettings();hideLoading();const {rental:r,lines,pays}=full;
    const deleteBtn=currentProfile?.role==='admin'?`<button class="btn btn-danger-outline delete-transaction-btn" onclick="deleteTransactionAdmin('${r.id}','${escapeHtml(r.bill_no)}')">🗑 Delete Transaction</button>`:'';
    el.innerHTML=`${billScreenHtml(r,lines,s)}${deleteBtn}${shareCustomerBox(r)}<div class="bill-actions"><button class="btn btn-whatsapp" onclick="shareBillPdf('${r.id}')">📄 Share Bill PDF</button><button class="btn btn-outline" onclick="printBill('${r.id}')">🖨️ ${t('print_bill')}</button></div><div class="section-label">${t('payments')}</div>${kpmPaymentQrActions(r)}<button class="btn btn-outline btn-block" style="margin-top:0" onclick="openDiscountForm('${r.id}',${Number(r.total_amount)},${Number(r.discount_amount||0)})">🏷️ ${t('discount')}</button>${pays.length?`<div class="section-label">${t('payment_history')}</div><div class="card">${pays.map(p=>`<div class="balance-box"><span class="l">${fmtDateTime(p.paid_at)}${p.note?' — '+escapeHtml(p.note):''}</span><span class="v">${money(p.amount)}</span></div>`).join('')}</div>`:''}`;
  }catch(e){hideLoading();el.innerHTML=`<div class="alert danger">${escapeHtml(e.message)}</div>`;}
}

/* ---------- Font size preference (per device/user browser) ---------- */
function applyKpmFontMode(mode){
  mode=['small','default','large'].includes(mode)?mode:'default';
  document.body.classList.remove('kpm-font-small','kpm-font-large');
  if(mode==='small')document.body.classList.add('kpm-font-small');
  if(mode==='large')document.body.classList.add('kpm-font-large');
  localStorage.setItem('kpm_font_mode',mode);
  document.querySelectorAll('[data-font-mode]').forEach(b=>b.classList.toggle('selected',b.dataset.fontMode===mode));
}
function kpmFontSettingsHtml(){
  const mode=localStorage.getItem('kpm_font_mode')||'default';
  return `<div class="font-setting-box"><div class="font-setting-title">Application Font Size</div><div class="font-setting-help">Change the overall reading size on this phone/device.</div><div class="font-setting-actions"><button class="btn btn-outline ${mode==='small'?'selected':''}" data-font-mode="small" onclick="applyKpmFontMode('small')">A− Small</button><button class="btn btn-outline ${mode==='default'?'selected':''}" data-font-mode="default" onclick="applyKpmFontMode('default')">A Default</button><button class="btn btn-outline ${mode==='large'?'selected':''}" data-font-mode="large" onclick="applyKpmFontMode('large')">A+ Large</button></div></div>`;
}

async function renderSettings(){
  const isAdmin=currentProfile?.role==='admin';showLoading();
  try{
    const s=await getShopSettings(true),card=document.getElementById('shopSettingsCard');
    card.innerHTML=(isAdmin?`<div class="field"><label>Owner Name</label><input id="setOwner" value="${escapeHtml(s.owner_name||'')}"></div><div class="field"><label>Owner Mobile</label><input id="setOwnerMobile" value="${escapeHtml(s.owner_mobile||'')}"></div><div class="field"><label>Store Address</label><textarea id="setAddress">${escapeHtml(s.address||'')}</textarea></div><div class="field"><label>UPI Name</label><input id="setUpiName" value="${escapeHtml(s.upi_name||'KPM')}"></div><button class="btn btn-brass btn-block" onclick="saveShopSettings()">Save Store Details</button>`:`<div class="store-detail"><b>${escapeHtml(s.owner_name||'')}</b><br>${escapeHtml(s.owner_mobile||'')}<br>${escapeHtml(s.address||'')}<br>UPI: ${escapeHtml(s.upi_name||'KPM')}</div>`)+kpmFontSettingsHtml();
    const img=document.getElementById('qrPreviewImg');if(s.qr_image_url){img.src=s.qr_image_url;img.style.display='block';}else img.style.display='none';
    const qrBtn=document.querySelector('#screen-settings button[onclick*="qrFileInput"]');if(qrBtn)qrBtn.style.display=isAdmin?'flex':'none';
  }catch(e){toast(e.message);}hideLoading();
}

/* Apply saved font mode as soon as this script loads. */
requestAnimationFrame(()=>applyKpmFontMode(localStorage.getItem('kpm_font_mode')||'default'));
