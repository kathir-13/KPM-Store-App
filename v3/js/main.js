...
import { applyStaticI18n } from './i18n/i18n.js';
...
function registerAllScreens(){
  ...
  registerScreen('new-rental', { render:()=>renderPlaceholder('screen-new-rental-inner','Stage 4'), title:'New Rental', inNav:false });
  registerScreen('active-rentals', { render:()=>renderPlaceholder('screen-active-rentals-inner','Stage 6'), title:'Active Rentals', inNav:false });
  registerScreen('edit-rental', { render:()=>renderPlaceholder('screen-edit-rental-inner','Stage 6'), title:'Edit Active Rental', inNav:false });
  registerScreen('returns', { render:()=>renderPlaceholder('screen-returns-inner','Stage 7'), title:'Return Items', inNav:false });
  registerScreen('payments', { render:()=>renderPlaceholder('screen-payments-inner','Stage 8'), title:'Payments', inNav:false });
  registerScreen('items', { render:()=>renderPlaceholder('screen-items-inner','Stage 3'), title:'Manage Items', inNav:false });
  registerScreen('transactions', { render:()=>renderPlaceholder('screen-transactions-inner','Stage 9'), title:'All Transactions', inNav:false });
  registerScreen('reports', { render:()=>renderPlaceholder('screen-reports-inner','Stage 9'), title:'Reports', inNav:false });
  registerScreen('users', { render:()=>renderPlaceholder('screen-users-inner','Stage 10'), title:'User Management', inNav:false });
  registerScreen('settings', { render:()=>renderPlaceholder('screen-settings-inner','Stage 10'), title:'Settings', inNav:false });
}
...
async function init(){
  ...
  initHeader({
    onLogout: goToLogin,
    onLangChange: ()=>{ /* re-render dynamic screens on language switch, expanded in later stages */ }
  });
